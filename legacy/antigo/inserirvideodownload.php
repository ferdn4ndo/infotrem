...
<title>Inserir V&iacute;deo Externo (Download)</title>
<link href="estilo.css" rel="stylesheet" type="text/css" />
</head>
<body><?
//Includes
include "includeslocalhost.php"; //Conecta ao banco de dados
include "videodownload.php"; //Faz o download do video
...
if(isset($_POST['txtLink'])) {
	...
	if($erro == 0){
		//Trata o nome para o arquivo
		$arquivo_minusculo = strtolower($txtNome);
		$caracteres = array("ç","~","^","]","[","{","}",";",":","´",",",">","<","-","/","|","@","$","%","ã","â","á","à","é","è","ó","ò","+","=","*","&","(",")","!","#","?","`","ã"," ","©");
		$arquivo_tratado = str_replace($caracteres,"_",$arquivo_minusculo);
		$caminho = "uploads/youtube/";
		
		//Faz o download
		$video_url = $txtLink;
		$filename = $caminho . $arquivo_tratado . ".flv";
		...
		if(mysql_num_rows(mysql_query($sqlverifica)) > 0) {
			...
			$sql = "INSERT INTO tblVideos (Categoria, Nome, Descricao, Keywords, CaminhoVideo, Externo) VALUES(" . $cmbCategoria . ",'" . $txtNome . "','" . $txtDescricao . "','" . $txtKeywords .  "','" . $filename . "',0);";		
			//echo "<br>" . $sql . "<br>";
			$ask = mysql_query($sql);
			if ($ask==0){
...
<form id="form1" name="form1" method="post" action="#"><div align="center">
  ...
  <tr>
      <td colspan="2" nowrap="nowrap" class="headertabela"><div align="center" class="header">Cadastrar V&iacute;deo Externo (Download)</div>
