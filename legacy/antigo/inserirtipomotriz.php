...
<title>Inserir Tipo de Automotriz ou Trens-Unidade</title>
...
if(isset($_POST['txtLetra'])) {
	...
	if(empty($txtLetra)){
		$msg = $msg . "Letras inv&aacute;lidas! <br>";
		$erro = $erro + 1;
	}
	if(empty($txtTipo)){
		$msg = $msg . "Tipo n&atilde;o preenchido!<br>";
		$erro = $erro + 1;
	}
	...
	if($erro == 0){
		...
		$sqlverifica = "SELECT * FROM tblTipoMotrizes WHERE TipoMotriz = '" . $letra . "';";
		
		if(mysql_num_rows(mysql_query($sqlverifica)) > 0) { //Se já houver um registro igual
			echo $Mensagem['erroduplicado'];
		} else {
			$sql = "INSERT INTO tblTipoMotrizes (TipoMotriz, Descricao) VALUES('" . $letra . "','" . $txtTipo . "');";
...
<form id="form1" name="form1" method="post" action="#"><div align="center">
  ...
  <tr>
    <td colspan="2" class="headertabela"><div align="center">Cadastrar Tipo de Automotriz ou Trem-unidade</div></td>
      </tr>
    <tr>
      <td width="119" class="headertabela"><div align="right">Sigla:</div></td>
      <td width="196" bgcolor="#FFFFFF"><span class="style2">
        <input name="txtLetra" type="text" id="txtLetra" size="10" maxlength="2" />
      </span></td>
    </tr>
    <tr>
