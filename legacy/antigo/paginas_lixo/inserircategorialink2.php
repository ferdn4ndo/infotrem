...
if($erro == 0){
	...
	$sqlverifica = "SELECT * FROM tblCategoriaLink WHERE Categoria = '" . $txtCategoria . "';";
	...
	if(mysqlcontar($id,$sqlverifica) > 0) { //Se já houver um registro igual
		...
		$sql = "INSERT INTO tblCategoriaLink (Categoria) VALUES('" . $txtCategoria . "');";
		
		//echo $sql;
		
		$ask = mysqlexecuta($id,$sql);
		if ($ask!=0){
			echo "<title>Inserir Categoria de Link</title>";
