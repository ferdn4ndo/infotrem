...
if($erro == 0){
	...
	$sqlverifica = "SELECT * FROM tblCategoriaLocal WHERE Categoria = '" . $txtCategoria . "';";
	...
	if(mysqlcontar($id,$sqlverifica) > 0) { //Se já houver um registro igual
		...
		$sql = "INSERT INTO tblCategoriaLocal (Categoria) VALUES('" . $txtCategoria . "');";
		
		//echo $sql;
		
		$ask = mysqlexecuta($id,$sql);
		if ($ask!=0){
			echo "<title>Inserir Categoria de Local</title>";
