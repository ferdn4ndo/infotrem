...
if(empty($txtPath)){
	$msg = $msg . "Caminho do documento inv&aacute;lido! <br>";
	$erro = $erro + 1;
}
...
if($erro == 0){
	...
	$sqlverifica = "SELECT * FROM tblDocumentos WHERE Categoria = " . $cmbCategoria . " AND Nome = '" . $txtNome . "';";
	...
	if(mysqlcontar($id,$sqlverifica) > 0) { //Se já houver um registro igual
		...
		$sql = "INSERT INTO tblDocumentos (Categoria, Nome, Keywords, Caminho, Detalhes) VALUES(" . $cmbCategoria . ",'" . $txtNome . "','" . $txtKeywords . "','" . $txPath .  "','" . $txtDetalhes . "');";
		
		//echo $sql;
		
		$ask = mysqlexecuta($id,$sql);
		if ($ask!=0){
			echo "<title>Inserir Documento</title>";
