...
if($erro == 0){
	...
	$sqlverifica = "SELECT * FROM tblCategoriaDocumento WHERE Categoria = '" . $txtCategoria . "';";
	
	//echo $sqlverifica;
	
	if(mysqlcontar($id,$sqlverifica) > 0) { //Se já houver um registro igual
		echo "<title>ERRO</title></head><body>";
		echo "<strong>J&aacute; existe uma categoria id&ecirc;ntica cadastrada no sistema!</strong>";
		exit;
	} else {
		$sql = "INSERT INTO tblCategoriaDocumento (Categoria) VALUES('" . $txtCategoria . "');";
		
		//echo $sql;
		
		$ask = mysqlexecuta($id,$sql);
		if ($ask!=0){
			echo "<title>Inserir Categoria de Documento</title>";
