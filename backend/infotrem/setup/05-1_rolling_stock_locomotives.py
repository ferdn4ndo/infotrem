...
def create_locomotive(model_name: str, sigo_number: int, **metadata):
    rolling_stock_item = RollingStock.objects.create(
    ...
    rolling_stock_locomotive = RollingStockLocomotive.objects.create(
...
def run():
    metric = TrackGaugeConfiguration.objects.get(tag='NARROW')
    gm = Manufacturer.objects.get(short_name='GM')
